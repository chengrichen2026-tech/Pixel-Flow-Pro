...
`Team Cloud：同伴 React 画布 → background 携成员令牌提交任务 → Tailscale HTTPS → 主机 127.0.0.1:43130 Team Gateway → Codex OAuth 生图 → background 轮询/重连 → 同伴 IndexedDB → React 画布`

`Team Web：同伴 React 画布 → 妙搭任务箱(provider=chatgpt_web) → Cloudflare 中继 → 已配对执行机 Pixel Flow background → 现有 ChatGPT contentScript → 任务箱结果分块 → 同伴 IndexedDB → React 画布`
