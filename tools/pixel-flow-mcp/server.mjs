...
const tools=[
 ...
 {name:"pixel_flow_get_products",description:"Read product-library item names and tags from the connected Pixel Flow extension.",inputSchema:{type:"object",properties:target,additionalProperties:false}},
 {name:"pixel_flow_get_library",description:"Read normalized prompt, product, or reference library records and missing asset ids.",inputSchema:{type:"object",properties:{scope:{type:"string",enum:["all","prompts","products","references"]},...target},additionalProperties:false}},
 {name:"pixel_flow_import_library",description:"Preview or apply agent-prepared prompt/product/reference records. Pixel Flow validates and writes supplied names, content, tags, and image data; it does not perform AI classification.",inputSchema:{type:"object",properties:{scope:{type:"string",enum:["prompts","products","references"]},mode:{type:"string",enum:["merge","replace"]},items:{type:"array",minItems:1,items:{type:"object",properties:{name:{type:"string"},content:{type:"string"},tags:{type:"array",items:{type:"string"}},imageDataUrl:{type:"string"},assetId:{type:"string"}},required:["name"],additionalProperties:false}},apply:{type:"boolean"},requestId:{type:"string"},expectedRevision:{type:"integer",minimum:0},...target},required:["scope","items","requestId","expectedRevision"],additionalProperties:false}},
 {name:"pixel_flow_auto_tag_products",description:"Infer product series and packaging tags from product names. Preview by default; set apply=true to write changes.",inputSchema:{type:"object",properties:{apply:{type:"boolean"},requestId:{type:"string"},expectedRevision:{type:"integer",minimum:0},...target},required:["requestId","expectedRevision"],additionalProperties:false}},
 ...
 {name:"pixel_flow_create_task",description:"Create a generation task with prompt, mode, team image model, ratio, position and optional source node ids.",inputSchema:{type:"object",properties:{prompt:{type:"string"},generationMode:{type:"string",enum:["api","browser","team","team_web"]},teamImageModel:{type:"string",enum:["flare","sunburst"]},aspectRatio:{type:"string"},x:{type:"number"},y:{type:"number"},sourceNodeIds:{type:"array",items:{type:"string"}},requestId:{type:"string"},expectedRevision:{type:"integer",minimum:0},...target},required:["requestId","expectedRevision"],additionalProperties:false}},
...
const call=async(name,args)=>{if(name==="pixel_flow_status")return text(await request("/health"));if(name==="pixel_flow_get_task")return text(await request(`/tasks/${encodeURIComponent(args.requestId)}`));const options={clientId:args.clientId,projectId:args.projectId,requestId:args.requestId,expectedRevision:args.expectedRevision};if(name==="pixel_flow_get_state")return text(await rpc("getState",{},options));if(name==="pixel_flow_get_products")return text(await rpc("getProducts",{},options));if(name==="pixel_flow_get_library")return text(await rpc("getLibrary",{scope:args.scope||"all"},options));if(name==="pixel_flow_import_library")return text(await rpc("execute",{commands:{op:"library.import",scope:args.scope,mode:args.mode||"merge",items:args.items,preview:!args.apply}},options));if(name==="pixel_flow_auto_tag_products")return text(await rpc("execute",{commands:{op:"library.autoTagProducts",preview:!args.apply}},options));if(name==="pixel_flow_execute")return text(await rpc("execute",{commands:args.commands},options));if(name==="pixel_flow_create_canvas")return text(await rpc("execute",{commands:{op:"canvas.create",name:args.name}},options));if(name==="pixel_flow_create_task")return text(await rpc("execute",{commands:{op:"task.create",prompt:args.prompt||"",generationMode:args.generationMode||"api",teamImageModel:args.teamImageModel||"flare",aspectRatio:args.aspectRatio||"auto",position:{x:args.x??620,y:args.y??220},sourceNodeIds:args.sourceNodeIds||[]}},options));if(name==="pixel_flow_run_task")return text(await rpc("execute",{commands:{op:"task.run",taskId:args.taskId}},options));if(name==="pixel_flow_download_image"){if(!path.isAbsolute(args.outputPath))throw new Error("outputPath must be absolute");const response=await rpc("getImage",{nodeId:args.nodeId},{...options,timeoutMs:60_000});const match=String(response.result?.dataUrl||"").match(/^data:([^;]+);base64,(.+)$/);if(!match)throw new Error("Pixel Flow returned invalid image data");await mkdir(path.dirname(args.outputPath),{recursive:true});await writeFile(args.outputPath,Buffer.from(match[2],"base64"));return text({ok:true,outputPath:args.outputPath,mimeType:match[1],bytes:Buffer.byteLength(match[2],"base64")})}throw new Error(`Unknown tool: ${name}`)};
